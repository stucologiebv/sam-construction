...
(function () {
  ...
  if (track && prevBtn && nextBtn && dotsWrap) {
    ...
    var visible = mobileQuery.matches ? 1 : 3;
    var current = 0;
    ...
    var dots = dotsWrap.querySelectorAll('.proj-dot');

    function goTo(idx) {
      current = ((idx % (maxPos + 1)) + (maxPos + 1)) % (maxPos + 1);
  ...
  if (filterWrap && gallery) {
    var cards = gallery.querySelectorAll('.proj-card');
    var filterBtns = filterWrap.querySelectorAll('.proj-filter');
    filterWrap.addEventListener('click', function(e) {
      var btn = e.target.closest('.proj-filter');
      if (!btn) return;
